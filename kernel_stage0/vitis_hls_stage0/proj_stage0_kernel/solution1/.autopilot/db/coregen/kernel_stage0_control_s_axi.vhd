...
architecture behave of kernel_stage0_control_s_axi is
    ...
    constant ADDR_Y_MEAN_DATA_0                : INTEGER := 16#118#;
    constant ADDR_Y_MEAN_DATA_1                : INTEGER := 16#11c#;
    constant ADDR_Y_MEAN_CTRL                  : INTEGER := 16#120#;
    constant ADDR_Y_REDUCE_DATA_0              : INTEGER := 16#124#;
    constant ADDR_Y_REDUCE_DATA_1              : INTEGER := 16#128#;
    constant ADDR_Y_REDUCE_CTRL                : INTEGER := 16#12c#;
    constant ADDR_Y_RELU_DATA_0                : INTEGER := 16#130#;
    constant ADDR_Y_RELU_DATA_1                : INTEGER := 16#134#;
    constant ADDR_Y_RELU_CTRL                  : INTEGER := 16#138#;
    constant ADDR_Y_EXPAND_DATA_0              : INTEGER := 16#13c#;
    constant ADDR_Y_EXPAND_DATA_1              : INTEGER := 16#140#;
    constant ADDR_Y_EXPAND_CTRL                : INTEGER := 16#144#;
    constant ADDR_Y_SIGMOID_DATA_0             : INTEGER := 16#148#;
    constant ADDR_Y_SIGMOID_DATA_1             : INTEGER := 16#14c#;
    constant ADDR_Y_SIGMOID_CTRL               : INTEGER := 16#150#;
    ...
    signal int_Y_mean          : UNSIGNED(63 downto 0) := (others => '0');
    signal int_Y_reduce        : UNSIGNED(63 downto 0) := (others => '0');
    signal int_Y_relu          : UNSIGNED(63 downto 0) := (others => '0');
    signal int_Y_expand        : UNSIGNED(63 downto 0) := (others => '0');
    signal int_Y_sigmoid       : UNSIGNED(63 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_Y_MEAN_DATA_0 =>
                        rdata_data <= RESIZE(int_Y_mean(31 downto 0), 32);
                    when ADDR_Y_MEAN_DATA_1 =>
                        rdata_data <= RESIZE(int_Y_mean(63 downto 32), 32);
                    when ADDR_Y_REDUCE_DATA_0 =>
                        rdata_data <= RESIZE(int_Y_reduce(31 downto 0), 32);
                    when ADDR_Y_REDUCE_DATA_1 =>
                        rdata_data <= RESIZE(int_Y_reduce(63 downto 32), 32);
                    when ADDR_Y_RELU_DATA_0 =>
                        rdata_data <= RESIZE(int_Y_relu(31 downto 0), 32);
                    when ADDR_Y_RELU_DATA_1 =>
                        rdata_data <= RESIZE(int_Y_relu(63 downto 32), 32);
                    when ADDR_Y_EXPAND_DATA_0 =>
                        rdata_data <= RESIZE(int_Y_expand(31 downto 0), 32);
                    when ADDR_Y_EXPAND_DATA_1 =>
                        rdata_data <= RESIZE(int_Y_expand(63 downto 32), 32);
                    when ADDR_Y_SIGMOID_DATA_0 =>
                        rdata_data <= RESIZE(int_Y_sigmoid(31 downto 0), 32);
                    when ADDR_Y_SIGMOID_DATA_1 =>
                        rdata_data <= RESIZE(int_Y_sigmoid(63 downto 32), 32);
...
-- ----------------------- Register logic ----------------
    ...
    Y_mean               <= STD_LOGIC_VECTOR(int_Y_mean);
    Y_reduce             <= STD_LOGIC_VECTOR(int_Y_reduce);
    Y_relu               <= STD_LOGIC_VECTOR(int_Y_relu);
    Y_expand             <= STD_LOGIC_VECTOR(int_Y_expand);
    Y_sigmoid            <= STD_LOGIC_VECTOR(int_Y_sigmoid);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_Y_MEAN_DATA_0) then
                    int_Y_mean(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_Y_mean(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_Y_MEAN_DATA_1) then
                    int_Y_mean(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_Y_mean(63 downto 32));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_Y_REDUCE_DATA_0) then
                    int_Y_reduce(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_Y_reduce(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_Y_REDUCE_DATA_1) then
                    int_Y_reduce(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_Y_reduce(63 downto 32));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_Y_RELU_DATA_0) then
                    int_Y_relu(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_Y_relu(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_Y_RELU_DATA_1) then
                    int_Y_relu(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_Y_relu(63 downto 32));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_Y_EXPAND_DATA_0) then
                    int_Y_expand(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_Y_expand(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_Y_EXPAND_DATA_1) then
                    int_Y_expand(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_Y_expand(63 downto 32));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_Y_SIGMOID_DATA_0) then
                    int_Y_sigmoid(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_Y_sigmoid(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_Y_SIGMOID_DATA_1) then
                    int_Y_sigmoid(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_Y_sigmoid(63 downto 32));
                end if;
            end if;
        end if;
    end process;
