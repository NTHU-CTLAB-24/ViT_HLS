...
#(parameter
    ...
    output wire [63:0]                   Y_mean,
    output wire [63:0]                   Y_reduce,
    output wire [63:0]                   Y_relu,
    output wire [63:0]                   Y_expand,
    output wire [63:0]                   Y_sigmoid,
...
localparam
    ...
    ADDR_Y_MEAN_DATA_0                = 9'h118,
    ADDR_Y_MEAN_DATA_1                = 9'h11c,
    ADDR_Y_MEAN_CTRL                  = 9'h120,
    ADDR_Y_REDUCE_DATA_0              = 9'h124,
    ADDR_Y_REDUCE_DATA_1              = 9'h128,
    ADDR_Y_REDUCE_CTRL                = 9'h12c,
    ADDR_Y_RELU_DATA_0                = 9'h130,
    ADDR_Y_RELU_DATA_1                = 9'h134,
    ADDR_Y_RELU_CTRL                  = 9'h138,
    ADDR_Y_EXPAND_DATA_0              = 9'h13c,
    ADDR_Y_EXPAND_DATA_1              = 9'h140,
    ADDR_Y_EXPAND_CTRL                = 9'h144,
    ADDR_Y_SIGMOID_DATA_0             = 9'h148,
    ADDR_Y_SIGMOID_DATA_1             = 9'h14c,
    ADDR_Y_SIGMOID_CTRL               = 9'h150,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_Y_mean = 'b0;
    reg  [63:0]                   int_Y_reduce = 'b0;
    reg  [63:0]                   int_Y_relu = 'b0;
    reg  [63:0]                   int_Y_expand = 'b0;
    reg  [63:0]                   int_Y_sigmoid = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_Y_MEAN_DATA_0: begin
                    rdata <= int_Y_mean[31:0];
                end
                ADDR_Y_MEAN_DATA_1: begin
                    rdata <= int_Y_mean[63:32];
                end
                ADDR_Y_REDUCE_DATA_0: begin
                    rdata <= int_Y_reduce[31:0];
                end
                ADDR_Y_REDUCE_DATA_1: begin
                    rdata <= int_Y_reduce[63:32];
                end
                ADDR_Y_RELU_DATA_0: begin
                    rdata <= int_Y_relu[31:0];
                end
                ADDR_Y_RELU_DATA_1: begin
                    rdata <= int_Y_relu[63:32];
                end
                ADDR_Y_EXPAND_DATA_0: begin
                    rdata <= int_Y_expand[31:0];
                end
                ADDR_Y_EXPAND_DATA_1: begin
                    rdata <= int_Y_expand[63:32];
                end
                ADDR_Y_SIGMOID_DATA_0: begin
                    rdata <= int_Y_sigmoid[31:0];
                end
                ADDR_Y_SIGMOID_DATA_1: begin
                    rdata <= int_Y_sigmoid[63:32];
                end
...
assign Y_mean                = int_Y_mean;
assign Y_reduce              = int_Y_reduce;
assign Y_relu                = int_Y_relu;
assign Y_expand              = int_Y_expand;
assign Y_sigmoid             = int_Y_sigmoid;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_Y_MEAN_DATA_0)
            int_Y_mean[31:0] <= (WDATA[31:0] & wmask) | (int_Y_mean[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_Y_MEAN_DATA_1)
            int_Y_mean[63:32] <= (WDATA[31:0] & wmask) | (int_Y_mean[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_Y_REDUCE_DATA_0)
            int_Y_reduce[31:0] <= (WDATA[31:0] & wmask) | (int_Y_reduce[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_Y_reduce[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_Y_REDUCE_DATA_1)
            int_Y_reduce[63:32] <= (WDATA[31:0] & wmask) | (int_Y_reduce[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_Y_RELU_DATA_0)
            int_Y_relu[31:0] <= (WDATA[31:0] & wmask) | (int_Y_relu[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_Y_RELU_DATA_1)
            int_Y_relu[63:32] <= (WDATA[31:0] & wmask) | (int_Y_relu[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_Y_EXPAND_DATA_0)
            int_Y_expand[31:0] <= (WDATA[31:0] & wmask) | (int_Y_expand[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_Y_expand[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_Y_EXPAND_DATA_1)
            int_Y_expand[63:32] <= (WDATA[31:0] & wmask) | (int_Y_expand[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_Y_sigmoid[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_Y_SIGMOID_DATA_0)
            int_Y_sigmoid[31:0] <= (WDATA[31:0] & wmask) | (int_Y_sigmoid[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_Y_sigmoid[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_Y_SIGMOID_DATA_1)
            int_Y_sigmoid[63:32] <= (WDATA[31:0] & wmask) | (int_Y_sigmoid[63:32] & ~wmask);
    end
end
