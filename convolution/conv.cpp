                ...
                In_channel:
                    ...
                    Kernel_Row:
                        ...
                        Kernel_Col:
                            for (int kernel_col = 0; kernel_col < KERNEL_SIZE; kernel_col++)
                            {
                                Out[batch][ch_out][row][col] += In[batch][ch_in][row * STRIDE + kernel_row][col * STRIDE + kernel_col] * Kernel[ch_out][kernelChannelIdx][kernel_row][kernel_col];
                            }
                        }
                        kernelChannelIdx++;
                    ...
                    if ((ch_out + 1) % outGroupNums == 0)
                        groupIndex++;
                    if (isBias)
                        Out[batch][ch_out][row][col] += Bias[ch_out];
                }
            }
        }
    }
