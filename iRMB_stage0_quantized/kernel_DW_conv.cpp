...
static void load_input(ftype *buffer_DataIn_1,
                       ftype in[BATCH_SIZE][CHANNEL_IN][HEIGHT_IN][WIDTH_IN])
{
init_in:
...
static void compute_padding(ftype in[BATCH_SIZE][CHANNEL_IN][HEIGHT_IN][WIDTH_IN],
                            ftype in_pad[BATCH_SIZE][CHANNEL_IN][HEIGHT_PAD][WIDTH_PAD])
{
Padding:
    for (int n = 0; n < BATCH_SIZE; n++)
    {
...
static void compute_conv(ftype in_pad[BATCH_SIZE][CHANNEL_IN][HEIGHT_PAD][WIDTH_PAD],
...
#pragma HLS LOOP_TRIPCOUNT min = CHANNEL_OUT max = CHANNEL_OUT
                    afterConv[n][c][i][j] = 0;
                    if (isConvBias)
                        afterConv[n][c][i][j] = (float)bias[c];
                }
            }
        }
    }
...
#pragma HLS LOOP_TRIPCOUNT min = CHANNEL_IN max = CHANNEL_IN
                                afterConv[batch][out_ch][row][col] += (float)(in_pad[batch][in_ch][row * STRIDE + kernel_row][col * STRIDE + kernel_col] * kernel[out_ch][kernelChannelIdx][kernel_row][kernel_col]);
                                kernelChannelIdx++;
...
static void compute_norm(float afterConv[BATCH_SIZE][CHANNEL_OUT][HEIGHT_OUT][WIDTH_OUT],
                         ftype afterNorm[BATCH_SIZE][CHANNEL_OUT][HEIGHT_OUT][WIDTH_OUT])
{
 float RUNNING_MEAN[CHANNEL_OUT];
#pragma HLS bind_storage variable=RUNNING_MEAN type=RAM_1P impl=uram
        float RUNNING_VAR[CHANNEL_OUT];
#pragma HLS bind_storage variable=RUNNING_VAR type=RAM_1P impl=uram
...
#pragma HLS LOOP_TRIPCOUNT min = WIDTH_OUT max = WIDTH_OUT
                for (int c = 0; c < CHANNEL_OUT; c++) {
#pragma HLS LOOP_TRIPCOUNT min = CHANNEL_OUT max = CHANNEL_OUT
                    afterNorm[n][c][i][j] = (ftype)((afterConv[n][c][i][j] - RUNNING_MEAN[c]) / sqrt(RUNNING_VAR[c] + EPS) * weight[c] + bias[c]);
                }
            }
        }
    }
...
static void compute_act(ftype afterNorm[BATCH_SIZE][CHANNEL_OUT][HEIGHT_OUT][WIDTH_OUT],
                        ftype *buffer_result)
{
...
extern "C"
{
    void kernel_DW_conv(ftype *buffer_DataIn_1,
                        ftype *buffer_result)
    {
// TODO: modify depth
...
#pragma HLS bind_storage variable=in_pad type=RAM_1P impl=uram
        float afterConv[BATCH_SIZE][CHANNEL_OUT][HEIGHT_OUT][WIDTH_OUT];
#pragma HLS bind_storage variable=afterConv type=RAM_1P impl=uram
        ftype afterNorm[BATCH_SIZE][CHANNEL_OUT][HEIGHT_OUT][WIDTH_OUT];
#pragma HLS bind_storage variable=afterNorm type=RAM_1P impl=uram
