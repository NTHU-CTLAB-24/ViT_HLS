...
static void compute_conv(ftype in[BATCH_SIZE][CHANNEL_IN][HEIGHT_IN][WIDTH_IN],
...
#pragma HLS LOOP_TRIPCOUNT min = CHANNEL_IN max = CHANNEL_IN
                        out[batch][out_ch][row][col] += (float)in[batch][in_ch][row][col] * kernel[out_ch][in_ch];
                    }
                    if (isConvBias)
...
extern "C"
{
    void kernel_proj(ftype *buffer_DataIn_1,
                     float *buffer_result)
    {
// TODO: modify depth
